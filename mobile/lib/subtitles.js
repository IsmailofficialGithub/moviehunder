...
export async function rawToSubtitleTrack(raw, label, source = "upload") {
  const vtt = toWebVtt(raw, label);
  return makeSubtitleTrack({ vttText: vtt, rawText: raw, label, source });
}

export async function uriToSubtitleTrack(uri, name) {
  const res = await fetch(uri);
  const bytes = await res.arrayBuffer();
  const raw = decodeSubtitleBytes(bytes);
  return rawToSubtitleTrack(raw, name || "Subtitles", "upload");
}
